...
public class MediaRailAdapter extends RecyclerView.Adapter<MediaRailAdapter.MediaRailViewHolder> {
  ...
  public MediaRailViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int type) {
    switch (type) {
      ...
      case TYPE_SEND:
        return new ButtonViewHolder(LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mediarail_sendleft_item, viewGroup, false));
      default:
  ...
  static class ButtonViewHolder extends MediaRailViewHolder {
    ...
    void bind(@Nullable RailItemAddListener addListener, boolean senkiri) {
      if (addListener != null) {
        itemView.setOnClickListener(v -> addListener.onRailItemAddClicked(senkiri));
      }
    }
    ...
    public interface RailItemAddListener {
      void onRailItemAddClicked(boolean senkiri);
    }
}
